...
(function() {
  ...
  document.querySelectorAll('[data-modal-open]').forEach(function(btn) {
    btn.addEventListener('click', function(e) {
      ...
      if (!id) return;
      openModal(id);
      // If this is the inquiry modal and the button carries a listing reference,
      // surface it in the modal header and store it for submission.
      if (id === 'modal-inquiry') {
        var ref = btn.getAttribute('data-inquiry-subject') || '';
        var refBlock = document.getElementById('pliListingRefBlock');
        var refText  = document.getElementById('pliListingRefText');
        var refInput = document.getElementById('pliListingReference');
        if (refInput) refInput.value = ref;
        if (refBlock && refText) {
          if (ref) { refText.textContent = ref; refBlock.hidden = false; }
          else { refBlock.hidden = true; refText.textContent = ''; }
        }
      }
    });
  });
